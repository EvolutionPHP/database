...
abstract class DB_query_builder extends DB_driver {
	...
	protected function _max_min_avg_sum($select = '', $alias = '', $type = 'MAX')
	{
		...
		{
			throw new \ErrorException('Invalid function type: '.$type);
		}
	...
	public function insert_batch($table, $set = NULL, $escape = NULL, $batch_size = 100)
	{
		...
		else
		{
			if (empty($set))
			{
				$this->display_error('insert_batch() called with no data');
			}
	...
	public function update_batch($table, $set = NULL, $index = NULL, $batch_size = 100)
	{
		...
		if ($index === NULL)
		{
			$this->display_error('db_must_use_index');
		}
		...
		else
		{
			if (empty($set))
			{
				$this->display_error('update_batch() called with no data');
			}
